...
ENV VIRTUAL_ENV /opt/snapenv-core-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/snapenv-core/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/snapenv-core/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/snapenv_core/ && touch src/snapenv_core/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/snapenv-core/
